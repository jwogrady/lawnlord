...
Superseded. The forward-looking plan is now the prerequisite chain in
[`../ROADMAP.md`](../ROADMAP.md) (v0.4.0–v0.8.0) and the GitHub issues assigned to each milestone —
which are the plan. This document is retained as the historical design record for the shipped
v0.2.0 (M1) work.
...
Resolved during v0.2.0/v0.3.0: M1 shipped as one milestone; the provider folder selects the adapter
while case identity comes from `caseNumber` (`25-09-14566`); and `knowledge_documents` stays
structure-only until the user supplies curated knowledge-base materials — now scoped in
[#41](https://github.com/jwogrady/lawnlord/issues/41).
